...
```bash
# 1. 在本地解压这次给你的压缩包
unzip Argo-reality-pqc-main-v2_3_2-tls13-pqc-hardened.zip -d argox
cd argox

# 2. 初始化 git 仓库
git init
git add -A
git commit -m "ArgoX v2.3.2: VLESS+XHTTP+PQC+ECH+Argo+CDN, TLS1.3 curve hardening"
...
```bash
git tag -a v2.3.2 -m "TLS1.3 + PQC curve hardening, complete XHTTP extra params"
...
3. 给这个 Tunnel 添加一条 **Public Hostname**，Hostname 必须和你等会
   要在脚本里填的固定域名**完全一致**（例如 `xhttp.example.com`）。
...
1. 固定 Tunnel 域名（B2 里绑定的那个，比如 `xhttp.example.com`）
2. 该 Tunnel 的 Token（隐藏输入，粘贴后回车）或者单行 credentials JSON

其余参数（`VLESS + XHTTP packet-up`、PQC 1-RTT、ECH、Cloudflare CDN、
随机 UUID/路径、自动避开被占用的本地端口）全部自动使用安全默认值。
这个模式会**拒绝**临时域名、无效凭据、以及覆盖已存在的 ArgoX 安装，
避免误装成弱配置。这是首次部署最推荐的方式。
...
如果你想自己指定路径、端口、PQC 严格模式等参数，用模板文件而不是 `-g`
（这一步需要本地有 `config-vless-xhttp-pqc-ech-argo-cdn.conf`，所以要先
`git clone` 整个仓库，或者单独下载这个模板文件）：
