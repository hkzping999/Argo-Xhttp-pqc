...
if ! (
  ...
  [[ "$ENABLE_VLESS_PQC" == 'y' && "$VLESS_PQC_CLIENT_RTT" == '1rtt' ]] &&
  [[ "$XHTTP_CDN_MODE" == 'packet-up' && "$ENABLE_ECH" == 'y' ]] &&
  [[ "$SERVER" == 'www.cloudflare.com' && "$SERVER_PORT" == '443' ]] &&
  [[ "$START_PORT" == '30001' && "$NGINX_PORT" == '8081' ]] &&
  [[ "$SERVER_IP" == '198.51.100.7' ]] &&
  [[ "$WS_PATH" =~ ^xh[0-9a-f]{16}$ ]]
); then
  fail 'guided installer defaults are incomplete or unsafe'
fi
...
if ! (
  ...
  TEST_GUIDED_CALLS=''
  reading() { printf -v "$2" '%s' 'XHTTP.EXAMPLE.COM'; }
  read() {
    local _target="${!#}"
  ...
  fail 'guided installer did not collect only the fixed domain and credentials'
fi
